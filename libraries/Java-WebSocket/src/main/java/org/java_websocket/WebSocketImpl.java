...
public class WebSocketImpl implements WebSocket {
				...
				if( role == Role.SERVER ) {
							...
							try {
								...
								if( handshakestate == HandshakeState.MATCHED ) {
									...
									try {
										...
										flushAndClose( CloseFrame.NEVER_CONNECTED, "WebSocketImpl: RunTimeException while decoding. Role server", false );
										return false;
									}
					...
					if( handshakestate == HandshakeState.MATCHED ) {
						try {
							...
							wsl.onWebsocketError( this, e );
							flushAndClose( CloseFrame.NEVER_CONNECTED, "WebSocketImpl: RunTimeException while decoding. Client role.", false );
							return false;
						}
						open( handshake );
						return true;
	...
	private void close( int code, String message, boolean remote ) {
		if( readystate != READYSTATE.CLOSING && readystate != READYSTATE.CLOSED ) {
			if( readystate == READYSTATE.OPEN ) {
				...
				flushAndClose( CloseFrame.NEVER_CONNECTED, "WebSocketImpl: Close without any message", false );
			}
	...
	protected void closeConnection( int code, boolean remote ) {
		closeConnection( code, "WebScoketImpl: eot. ReadyState Not Yet Connected", remote );
	}
	...
	public void close( InvalidDataException e ) {
		System.out.println("fadil Closing the WS " + e.getMessage());
